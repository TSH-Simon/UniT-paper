...
from bert_cnn_classifier_2_gauss_linear import textcnn_lipdy_bert
import argparse
import glob
import logging
import os
...
from bert_cnn_classifier_2_gauss_linear.bert_cnnencoder import bert_cnnencoder
...
def train(args, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            orig_batch = examples[batch[4].tolist()]
            orig_batch = tuple(t.to(args.device) for t in orig_batch)

            inputs = {'input_ids':      batch[0],
                      'input_orig_ids': orig_batch[0],
...
def load_and_cache_examples_randomized(args, task, tokenizer, random_smooth, epoch, evaluate=False):
    ...
    all_index = torch.tensor([i for i in range(len(features))], dtype=torch.long)
...
def main():
    ...
    parser.add_argument("--norm_weight", default=1.0, type=float,
                        help="The weight of loss norm.")
    parser.add_argument("--margin", default=0.7, type=float,
                        help="The margin for loss.")
    parser.add_argument("--var", default=0.1, type=float,
                        help="variance.")
    parser.add_argument("--alpha_weight", default=0.7, type=float,
                        help="weight.")
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        save_model(load_dict_bert, args.output_dir)
        print("model saved")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        path = os.path.join(args.output_dir, 'model' + '.pth')
        print('loading from {}'.format(path))
